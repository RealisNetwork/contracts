...
pub const LOOCKUP_ACCOUNT: &str = "lockup.v1.realisnetwork.near";
pub const TOKEN_ACCOUNT: &str = "token.v1.realisnetwork.near";
...
pub async fn pull_contract(worker: &Worker<Sandbox>) -> anyhow::Result<Contract> {
    ...
    let contract_id: AccountId = LOOCKUP_ACCOUNT.parse()?;
    let contract = worker
    ...
    contract
        .call("new")
        .args_json(serde_json::json!({ "token_account_id": TOKEN_ACCOUNT, "deposit_whitelist": [LOOCKUP_ACCOUNT] }))
        .transact()
        .await?
        .into_result()?;
