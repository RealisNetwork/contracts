...
use near_sdk::{AccountId, PublicKey, require};
...
impl Contract {
    ...
    pub fn internal_transfer(&mut self, sender: AccountId, recipient_id: AccountId, amount: u128) -> u128 {
    ...
    pub fn take_fee(&mut self, sender: AccountId, amount: Option<u128>) -> u128{
        ...
        let (charge, fee) = if amount.is_some() {
            // TODO: use U256
            ((amount.unwrap() * (self.percent_fee as u128 + 100))/100, (amount.unwrap() * self.percent_fee as u128)/100)
        } else {
            (self.constant_fee, self.constant_fee)
        };

        // Check if user exists and get account, if user don't exist, rollback transfer
        let mut sender_account = self.accounts.get(&sender).unwrap_or_else(|| env::panic_str("User not found"));

        // Check if user have enough tokens to send
        require!(sender_account.free >= amount.unwrap_or_default() , "Not enough balance");
        ...
        self.accounts.insert(&self.beneficiary_id, &beneficials_account);
...
mod tests {
    use std::str::FromStr;
    use near_sdk::{Gas, testing_env, VMContext};
    ...
    fn transfer() {
        ...
        contract.accounts.insert(&AccountId::from_str(sender_id).unwrap(), &Account { free: 250 }); // Will be 228

        // reciever
        let reciever_id = "mike.testnet";
        contract.accounts.insert(&AccountId::from_str(reciever_id).unwrap(), &Account { free: 9 }); // Will be 29

        contract.internal_transfer(AccountId::from_str(sender_id).unwrap(), AccountId::from_str(reciever_id.clone()).unwrap(), 20);

        assert_eq!(contract.accounts.get(&contract.beneficiary_id.clone()).unwrap().free, 2);
        assert_eq!(contract.accounts.get(&AccountId::from_str(sender_id).unwrap()).unwrap().free, 228);
        assert_eq!(contract.accounts.get(&AccountId::from_str(reciever_id).unwrap()).unwrap().free, 29);
    }
    ...
    fn transfer_not_enough_balance() {
        ...
        contract.internal_transfer(AccountId::from_str(sender_id).unwrap(), AccountId::from_str(reciever_id.clone()).unwrap(), 251);

        assert_eq!(contract.accounts.get(&contract.beneficiary_id.clone()).unwrap().free, 0);
        assert_eq!(contract.accounts.get(&AccountId::from_str(sender_id).unwrap()).unwrap().free, 250);
        assert_eq!(contract.accounts.get(&AccountId::from_str(reciever_id).unwrap()).unwrap().free, 9);
    }
    ...
    fn transfer_cant_pay_fees() {
        ...
        contract.internal_transfer(AccountId::from_str(sender_id).unwrap(), AccountId::from_str(reciever_id.clone()).unwrap(), 250);
    ...
    fn transfer_zero() {
        ...
        // Sender
        let sender_id = "gregory.testnet";
        contract.accounts.insert(&AccountId::from_str(sender_id).unwrap(), &Account { free: 250 });

        // reciever
        let reciever_id = "mike.testnet";
        contract.accounts.insert(&AccountId::from_str(reciever_id).unwrap(), &Account { free: 9 }); // Will be 29

        contract.internal_transfer(AccountId::from_str(sender_id).unwrap(),
                                   AccountId::from_str(reciever_id.clone()).unwrap(), 0); // TRY SEND INVALID BALANCE
    ...
    fn transfer_to_no_account() {
        ...
        assert_eq!(contract.accounts.get(&contract.beneficiary_id.clone()).unwrap().free, 2);
        assert_eq!(contract.accounts.get(&AccountId::from_str(sender_id).unwrap()).unwrap().free, 228);
        assert_eq!(contract.accounts.get(&AccountId::from_str(reciever_id).unwrap()).unwrap().free, 20);
    }
}